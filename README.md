...
Three dashboards for Actibase, each implementing a Claude Design prototype
...
Open any of the three HTML files directly in a browser, or serve the
...
- `index.html` / `coach.html` / `player.html` — page shells; each loads its own scripts below
...
- `js/app.js` / `js/coach-app.js` / `js/player-app.js` — app state, render
  functions per page/modal, and event delegation (`data-action`
  attributes) that drives everything — no framework, just a mutable
  `state` object and a `render()` that rebuilds the DOM from template
  strings on every change

Each module is a fully independent page load (its own `<script>` set),
so the three `state`/`render`/`actions` globals never collide even
though they share the same names — only `css/tokens.css`,
