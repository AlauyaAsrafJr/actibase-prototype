...
from ..serializers import account_name, announcement_out, full_name, player_out, user_out
from ..utils import (
    REPORT_RANGES,
    TODAY,
    attendance_pct,
...
def _admin_reports_query(db, sport: str | None):
    q = (
    ...
    return q.order_by(models.ReportAnalytics.report_id.desc())


def _report_out(db, r: models.ReportAnalytics) -> schemas.ReportOut:
    return schemas.ReportOut(
        id=r.report_id, name=r.title, sport=r.sport, range=r.range, generated_on=r.generated_date,
        status=r.status, details=r.details, generated_by_name=account_name(db, db.get(models.SystemUser, r.generated_by)),
    )
...
def list_reports():
    db = get_db()
    reports = _admin_reports_query(db, request.args.get("sport")).all()
    return json_response([_report_out(db, r).model_dump() for r in reports])
...
def generate_report():
    ...
    db.commit()
    db.refresh(report)
    return json_response(_report_out(db, report), 201)


@admin_bp.get("/reports/summary")
def reports_summary():
    db = get_db()
    reports = _admin_reports_query(db, None).all()
    dates = [r.generated_date for r in reports]

    this_month = sum(1 for d in dates if (parsed := parse_date(d)) and parsed.year == TODAY.year and parsed.month == TODAY.month)

    active_season = db.query(models.Season).filter(models.Season.is_active.is_(True)).first()
    this_term = None
    if active_season is not None:
        window = season_window(active_season)
        if window is not None:
            this_term = sum(1 for d in dates if (parsed := parse_date(d)) and window[0] <= parsed <= window[1])

    sport_counts: dict[str, int] = {}
    for r in reports:
        if r.sport and r.sport != "All sports":
            sport_counts[r.sport] = sport_counts.get(r.sport, 0) + 1
    by_sport = sorted((schemas.SportCount(sport=k, count=v) for k, v in sport_counts.items()), key=lambda s: -s.count)

    out = schemas.ReportsSummaryOut(
        total=len(reports),
...
def export_reports_csv():
    ...
    writer.writerow(["Report ID", "Name", "Sport", "Range", "Generated", "Status", "Generated By"])
    for r in reports:
        writer.writerow([r.report_id, r.title, r.sport, r.range, r.generated_date, r.status, account_name(db, db.get(models.SystemUser, r.generated_by))])

    return Response(buffer.getvalue(), mimetype="text/csv", headers={"Content-Disposition": "attachment; filename=actibase-reports.csv"})
